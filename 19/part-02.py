...
if __name__ == "__main__":
	...
	known_sensors = list( sorted( known_sensors, key = lambda x: x[ "name" ] ) )

	for i in range( len( known_sensors ) - 1 ):
		s1 = known_sensors[ i ]

		x1, y1, z1 = s1[ "relative" ]

		for s2 in known_sensors[ i+1: ]:
			x2, y2, z2 = s2[ "relative" ]

			manhattan_distance = abs( x1 - x2 ) + abs( y1 - y2 ) + abs( z1 - z2 )

			print( "  {} -> {}: {}".format( s1[ "name" ], s2[ "name" ], manhattan_distance ) )

			if manhattan_distance > largest_distance:
				largest_distance = manhattan_distance

	print( "\n\nLargest distance between two sensors is: {}".format( largest_distance ) )
